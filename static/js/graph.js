class Graph {
  ...
  removeNode(nodeId) {
    for (let otherNode in this.adjacencyList) {
      this.adjacencyList[otherNode] = this.adjacencyList[otherNode].filter(
        (node) => node !== nodeId
      );
    }
...
class CytoscapeGraph {
  constructor(container, graph) {
    ...
    this.cy.on("tap", "node", (evt) => {
      // arrow function here
      console.log("add edge mode: " + addEdgeMode);
      if (addEdgeMode) {
        let node = evt.target;
        ...
        if (selectedNodes.length >= 2) {
          ...
          let edgeId =
            "edge" + selectedNodes[0].id() + "to" + selectedNodes[1].id();
  ...
  update() {
    ...
    for (let nodeId in adjacencyList) {
      if (!this.cy.$id(nodeId).nonempty()) {
        this.cy.add({ data: { id: nodeId, label: nodeId } });
      }
    }
    for (let nodeId in adjacencyList) {
      adjacencyList[nodeId].forEach((neighborId) => {
        ...
        if (this.graph.isDirected || !this.cy.$id(reverseEdgeId).nonempty()) {
          let edgeData = {
            ...
            source: nodeId,
            target: neighborId,
            curveStyle: !this.cy.$id(reverseEdgeId).nonempty()
              ? "bezier"
              : "haystack",
